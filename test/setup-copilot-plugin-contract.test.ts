...
describe('setup: Copilot local plugin contract', () => {
  ...
  test('setup auto-detect notices Copilot CLI without silently installing it', () => {
    ...
    expect(autoExecutableLines.some(line => /command -v copilot|copilot [^\n]*(?:--version|version|help)/.test(line))).toBe(true);
    expect(autoExecutableLines.some(line => /(?:HOST="?copilot"?|INSTALL_[A-Z_]*COPILOT[A-Z_]*=1|install_[a-z_]*copilot|copilot plugin install)/i.test(line))).toBe(false);
    expect(autoBlock).toMatch(/--host copilot/);
  });
